...
> 프로젝트 본체는 [`jlens_repro/`](jlens_repro/) 아래에 있습니다. 상세 문서는
> [`jlens_repro/README.md`](jlens_repro/README.md), 테스터 사용법은
> [`jlens_repro/TOOL.md`](jlens_repro/TOOL.md) 참조.
...
- 자세한 내용: [`jlens_repro/results/REPORT.md`](jlens_repro/results/REPORT.md),
  [`jlens_repro/results/scale_compare.md`](jlens_repro/results/scale_compare.md),
  [`jlens_repro/results/e2e_run_report.md`](jlens_repro/results/e2e_run_report.md).
...
# 기반 라이브러리 클론 + 설치 (.gitignore 처리되어 이 레포엔 포함 안 됨)
git clone https://github.com/anthropics/jacobian-lens
pip install ./jacobian-lens
```
...
브라우저에서 **http://127.0.0.1:8137** 접속. 자세한 사용법은
[`jlens_repro/TOOL.md`](jlens_repro/TOOL.md).
...
- **비밀은 커밋되지 않습니다.** 실제 API 키는 `.env`(→ `.gitignore`)에만 있고, 저장소에는
  값이 빈 [`jlens_repro/.env.example`](jlens_repro/.env.example)만 포함됩니다.
