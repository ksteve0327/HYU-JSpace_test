...
<h2>부록 A — 관련 연구(ICML)와의 대비 · Q&amp;A</h2>
<p class="mut">이 실험 뒤 나온 문답. 관련 포스터: <b>“Invisible Conflicts: Media Coverage Asymmetry
and Categorical Failure in LLM Conflict Forecasting”</b> (ICML 2026 Spotlight) — LLM이 보도가 적은
분쟁지역을 “예측”하는 게 아니라 <b>카테고리 라벨을 암송</b>한다는 결과(Llama가 under-covered 지역에
전부 YES, R=0.918; “6년간 매달 격화 예측 = 예측이 아니라 카테고리 암송”).</p>

<div class="lead" style="border-left-color:var(--green)"><b>Q (사용자 가설 사슬).</b>
“시그널 강함=학습 데이터 많음=추론 정보 많음 → 다양한 답변” vs “학습 데이터 없음=추론 정보 적음 →
<b>일관적 답변</b>(ICML)”. 즉 저시그널일수록 “가장 가까운 아는 것”을 읊어 일관적이다 — 이 사슬이 맞나?
(그리고 주식에서 쓰던 naive-baseline 지표를 여기서도 쓰는 이유)</div>

<h3>A1. ICML 세팅에선 맞고, 이 실험(개방형 생성)에선 반대</h3>
<p>당신 사슬은 <b>ICML(이진 예측)과는 일치</b>하지만, 우리가 직접 측정한 Phase 6과는 <b>정반대</b>다.
두 결과의 방향이 뒤집혀 있다:</p>
<table><tr><th></th><th>아는 것 (고시그널)</th><th>모르는 것 (저시그널)</th></tr>
<tr><td>사용자 사슬 / ICML</td><td>다양</td><td><b>일관 (암송)</b></td></tr>
<tr><td><b>Phase 6 (측정값)</b></td><td><b>일관</b> (Paris→늘 France, distinct 1/10)</td>
  <td><b>다양</b> (Trennsk→유럽국가 산발 9/10, 엔트로피 5.89)</td></tr></table>

<h3>A2. 왜 갈리나 — 핵심: prior의 “모양” + 답변 공간</h3>
<p>둘을 묶는 법칙은 <b>“저시그널 → prior(사전분포)로 후퇴”</b>. 다만 prior의 모양이 겉모습을 가른다:</p>
<div class="cards">
 <div class="card"><b>ICML — 일관(암송)</b>답이 <b>YES/NO 이진</b> + “분쟁지역=격화”라는 <b>뾰족한 카테고리
   prior</b> → 후퇴하면 그 한 라벨로 수렴.</div>
 <div class="card"><b>Phase 6 — 확산</b>답이 <b>개방형</b>(아무 나라나) + 지어낸 이름엔 <b>암송할 카테고리가
   없음</b> → 후퇴하면 “그럴듯한 지명” 넓은 분포로 확산.</div>
</div>
<p>즉 “없으면 가장 가까운 걸 읊는다”는 <b>prior가 뾰족할 때만</b> 성립. 넓으면 “가장 가까운 하나”가
없어 방황한다. <b>결정 변수는 데이터 양이 아니라 ① 암송할 카테고리 유무 ② 답변 공간이 닫혔는지.</b></p>

<h3>A3. 가장 약한 고리 — “유사 토큰 적다/많다”</h3>
<p>이 중간 고리는 우리가 <b>직접 측정(이웃 응집도)했는데 친숙도와 무관하게 평탄</b>(0.30–0.32)했다.
방향도 직관과 반대(“데이터 많다→유사토큰 적다”)이고, <b>J-space는 저장고가 아니라 렌즈</b>라 유사토큰
수로 인과를 세우는 건 위험하다. 실제 구동인은 유사토큰 수가 아니라 <b>prior 모양 + 과제 구조</b>였다.
→ 이 고리는 제거 권장.</p>

<h3>A4. “주식 지표” — Always-YES = 랜덤워크 baseline</h3>
<p>ICML의 <b>Always-YES 베이스라인</b> = “기저율을 그냥 읊는” 순진한 예측기 = 금융의 <b>랜덤워크/naive
baseline</b>과 같은 역할. 이걸 못 이기면 “예측”이 아니라 categorize다. <b>바로 이래서 주가 예측이
J-space 검정에 부적합</b>했다(랜덤워크 = 못 이기는 naive baseline → “제한적 출력”이 정보 부재 때문인지
원래 예측불가 때문인지 분리 불가). Phase 6에서 주가를 뺀 판단이 옳았음을 ICML이 뒷받침한다.</p>

<h3>A5. 결론</h3>
<ul>
 <li>사용자 사슬: <b class="verdict supported">ICML(이진+카테고리 prior)에선 참</b>,
   <b class="verdict refuted">개방형 생성에선 거짓</b> — 보편 법칙 아님.</li>
 <li>올바른 일반 진술: <b>“저시그널 → prior 암송; prior가 뾰족하면 일관(ICML), 넓으면 확산(Phase 6).”</b></li>
 <li>두 연구를 함께 쓰면: “모델이 저시그널에서 <b>reason 대신 recite</b>한다”는 현상은 실재하되, 그 겉모습
   (일관 vs 확산)은 <b>과제 구조에 의존</b> — ICML(분쟁·이진)과 Phase 6(지명·개방)이 그 두 얼굴.</li>
</ul>
