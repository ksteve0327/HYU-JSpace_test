...
- 부가: AI 해석(로컬 claude-proxy 또는 OpenRouter)·챗봇·자동반복·로컬 기록·전체 자동 실행
...
- **claude-proxy는 로컬 전용.** AI 해석의 기본 경로인 `localhost:11436` claude-proxy는
  코드에 박힌 **로컬 주소**일 뿐 외부로 나가지 않으며, 인증 키가 없습니다. 프록시가 안 떠
  있으면 자동 비활성화됩니다.
